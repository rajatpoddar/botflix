...
export default function MoviesPage() {
  // Library folders (Bollywood, Hollywood, South…)
  const [libraries, setLibraries] = useState([])
  // Map of libraryId -> items[] for each carousel
  const [libraryItems, setLibraryItems] = useState({})
  const [hero, setHero] = useState([])
  const [loading, setLoading] = useState(true)

  // Grid state — "all" or a specific library id
  ...
  // Step 1: load library list + per-library carousels
  useEffect(() => {
    async function init() {
      try {
        ...
        // Fetch first 20 movies from each library in parallel
  ...
  // Step 2: load grid when filter/sort/page changes
  useEffect(() => {
  ...
  const activeLibName = activeLib === 'all'
    ? 'All Movies'
    : libraries.find((l) => l.Id === activeLib)?.Name || 'Movies'
